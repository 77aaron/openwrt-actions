...
sed -i '/uci commit system/i\uci set system.@system[0].hostname='Aaron_OpenWrt'' package/lean/default-settings/files/zzz-default-settings

# 版本号里显示一个自己的名字（281677160 build $(TZ=UTC-8 date "+%Y.%m.%d") @ 这些都是后增加的）
sed -i "s/OpenWrt /Aaron build $(TZ=UTC-8 date "+%Y.%m.%d") @ OpenWrt /g" package/lean/default-settings/files/zzz-default-settings
...
sed -i 's/"TTYD 终端"/"终端"/g' package/lean/luci-app-ttyd/po/zh-cn/terminal.po
